...
export default function Profile(){
    ...
    useEffect(()=>{
        if(userInfo.profileSetUp){
            ...
            setSelectedColored(userInfo.color)
        }
        if(userInfo.image){
    ...
    const saveChanges = async()=>{
       if(validateProfile()){
        try {
            const response = await apiClient.post(`${UPDATE_PROFILE_ROUTE}`,{firstName,lastName,color:selectedColored},{withCredentials:true})
         ...
         if(file){
            ...
            formData.append("profile-image",file);
             ...
             if(response.status===200 && response.data.image){
                 ...
                 toast.success("Profle image set successfully")
             }
         }
    };
        ...
        <div className="bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10">
            <div className="flex flex-col gap-10 w-[80vw] md:w-max">
                <div onClick={handleNavigate}>
                    <IoArrowBack className="text-4xl lg:text-6xl text-white/90 curser-pointer"/>
                </div>
                <div className="grid grid-cols-2 ">
                   ...
                   >
                      <Avatar className="h-32 w-32 md:w-48 md:h-48 rounded-full overflow-hidden">
                         {
                            ...
                            <div className={`uppercase w-32 h-32 md:w-48 md:h-48 text-5xl border-[1px] flex items-center justify-center rounded-full ${getColors(selectedColored)}`}>
                      ...
                      <input type="file"className="hidden" ref={fileInputRef} onChange={handleImageChange} name="profile-image" accept=".png,.jpeg,.webp,.jpg,.svg"/>
                   </div>
                   <div className="flex min-w-32 md:min-w-64 flex-col gap-5 text-white items-center justify-center ">
                        ...
                        <div className="w-full flex gap-5 ">
                           {colors.map((color,index)=> <div className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all duration-300 ${selectedColored === index ? "outline outline-white outline-1":""}`} key={index}
                           onClick={()=>setSelectedColored(index)}
                           >
